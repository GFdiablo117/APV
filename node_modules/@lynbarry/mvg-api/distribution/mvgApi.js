...
var mvgHeader = {
     ...
     */
};var departureIdEndpoint = function departureIdEndpoint(id) {
    return "https://www.mvg.de/fahrinfo/api/departure/" + id + "?footway=0";
};
 ...
 */
var stationEndpoint = function stationEndpoint(identifier) {
    return "https://www.mvg.de/fahrinfo/api/location/query?q=" + encodeURI(identifier);
};
...
function getDepartures(stationName, transportTypes, apiRedirectUrl) {
    return getStationId(stationName, apiRedirectUrl).then(function (stationId) {
        return getDeparturesById(stationId, transportTypes, apiRedirectUrl);
    });
}

function getStationId(stationName, apiRedirectUrl) {
    return getStationByName(stationName, apiRedirectUrl).then(function (station) {
        return station.id;
    });
}

function getStationByName(stationName, apiRedirectUrl) {
    var requestUri = stationEndpoint(stationName);

    if (typeof apiRedirectUrl == "string") {
        requestUri = apiRedirectUrl + "/" + requestUri;
    }

    return requestPromise(requestUri).then(function (requestBody) {
        return handleJSON(requestBody);
    }).then(function (jsonBody) {
        return getStationFromJSON(jsonBody);
    });
}

function requestPromise(requestUri) {
    return new Promise(function (resolve, reject) {
        request.get({ uri: requestUri, encoding: null, headers: mvgHeader }, function (error, response, body) {
...
function getStationFromJSON(jsonBody) {
    return new Promise(function (resolve, reject) {
        try {
            var station = jsonBody.locations[0];
            resolve(station);
        } catch (exception) {
...
function getDeparturesById(stationId, transportTypes, apiRedirectUrl) {
    var requestUri = departureIdEndpoint(stationId);
    ...
    return requestPromise(requestUri).then(function (requestBody) {
        ...
        return getLinesFromJSON(jsonBody, transportTypes);
    });
}

function getLinesFromJSON(jsonBody, transportTypes) {
    return new Promise(function (resolve, reject) {
        var departures = jsonBody.departures;
        try {
            var lines = convertDeparturesToLine(departures);
            var filteredLines = filterLines(lines, transportTypes);
